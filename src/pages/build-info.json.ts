...
export const GET: APIRoute = async () => {
  ...
  return new Response(JSON.stringify({
    ...
    counts: { roles: (await loadRoles()).length, entries: entries.length, skills: entries.filter(e => e.kind === 'skill').length, tools: entries.filter(e => e.kind === 'tool').length, bundles: bundles.length, categories: (await loadCategories()).length },
