...
export const GET: APIRoute = async ({ site }) => {
  ...
  const paths = ['examples/research-to-delivery/', 'categories/', ...categoryPaths, '', 'skills/', 'tools/', 'workflows/', 'about/', 'bundles/', ...bundles.map(b=>`bundles/${b.id}/`), ...entries.map(e=>`${e.kind === 'skill' ? 'skills' : e.kind === 'tool' ? 'tools' : 'workflows'}/${e.id}/`)];
