...
if (root) {
  ...
  function render(writeURL = true) {
    ...
    let visible = 0;
    cards.forEach(card => {
      const match = terms.every(term => normalize(card.dataset.search ?? '').includes(term)) && (kind === 'all' || card.dataset.kind === kind) && (category === 'all' || card.dataset.category === category) && (origin.value === 'all' || card.dataset.origin === origin.value);
    ...
    if (writeURL) {
      ...
      for (const [key,value] of Object.entries({q:search.value.trim(),kind,category,origin:origin.value})) {
  ...
  root.querySelector('#clear-filters')?.addEventListener('click',()=>{search.value='';origin.value='all';kind='all';category='all';render();});
