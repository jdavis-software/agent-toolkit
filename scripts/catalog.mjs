...
export function validateEntries(entries) {
  ...
  for (const e of entries) {
    ...
    ids.add(e.id);
    if(e.companionTools!==undefined && (!Array.isArray(e.companionTools)||e.companionTools.some(id=>!['agentflow','sourcekit','harnesskit'].includes(id))||new Set(e.companionTools).size!==e.companionTools.length))throw new Error(`Invalid companion tools: ${e.id}`);
