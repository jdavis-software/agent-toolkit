...
export async function main(args=process.argv.slice(2)) {
  ...
  if(!['list','resolve'].includes(command))throw new Error('Unknown command');
  const id=command==='resolve'?args.shift():undefined;
  ...
  for(let i=0;i<args.length;i++) {
    ...
    seen.add(f);
    if(command==='list'&&f!=='--root')throw new Error('Option is only supported by resolve');
  ...
  const result=command==='list'?{schemaVersion:1,bundles:await loadBundles(root)}:await resolveBundle(root,id,{requireClean,expectedRevision});
