...
test('thirteen primary categories partition all 81 entries without duplicate counts',async()=>{
 const cs=await loadCategories();assert.equal(cs.length,13);assert.equal(cs.reduce((n,c)=>n+c.counts.entries,0),81);
 assert.equal(cs.reduce((n,c)=>n+c.counts.skills,0),73);assert.equal(cs.reduce((n,c)=>n+c.counts.tools,0),8);
 assert.equal(new Set(cs.flatMap(c=>c.entryIds)).size,81);
});
