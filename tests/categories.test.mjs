...
test('thirteen primary categories partition all 73 entries without duplicate counts',async()=>{
 const cs=await loadCategories();assert.equal(cs.length,13);assert.equal(cs.reduce((n,c)=>n+c.counts.entries,0),73);
 assert.equal(cs.reduce((n,c)=>n+c.counts.skills,0),66);assert.equal(cs.reduce((n,c)=>n+c.counts.tools,0),7);
 assert.equal(new Set(cs.flatMap(c=>c.entryIds)).size,73);
});
