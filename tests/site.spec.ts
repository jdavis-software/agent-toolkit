...
test('personal collection renders without errors or horizontal overflow',async({page},testInfo)=>{
  ...
  await expect(page.locator('[data-entry]:visible')).toHaveCount(41);
  await expect(page.locator('#result-count')).toHaveText('Showing 41 of 41 entries');
...
test('search, reset, category and origin filters work',async({page})=>{
  ...
  await expect(page.locator('[data-entry]:visible')).toHaveCount(38);
...
test('catalog export and all local navigation targets resolve',async({page,request})=>{
  ...
  expect(entries.filter((entry:{origin:string})=>entry.origin==='original')).toHaveLength(38);
  await page.goto(base);
...
test('skills are local packages with working instructions',async({page})=>{
  ...
  await expect(page.locator('[data-entry]')).toHaveCount(36);
...
test('bundle navigation links to canonical skills and a read-only command',async({page,request},testInfo)=>{
 ...
 await expect(page.locator('.bundle-card')).toHaveCount(8);
 expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBeTruthy();
 ...
 const bundles=(await data.json()).bundles;expect(bundles).toHaveLength(8);
 expect(new Set(bundles.flatMap((b:{skills:string[]})=>b.skills)).size).toBe(36);
});

test('original executable tools are not mislabeled upstream skills',async({page})=>{
 for(const id of ['skillcheck','bundle-resolver']){
