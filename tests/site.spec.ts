...
test('personal collection renders without errors or horizontal overflow',async({page},testInfo)=>{
  ...
  await expect(page.locator('[data-entry]:visible')).toHaveCount(121);
  await expect(page.locator('#result-count')).toHaveText('Showing 121 of 121 entries');
...
test('search, reset, category and origin filters work',async({page})=>{
  ...
  await expect(page.locator('[data-entry]:visible')).toHaveCount(118);
...
test('catalog export and all local navigation targets resolve',async({page,request})=>{
  ...
  expect(entries.filter((entry:{origin:string})=>entry.origin==='original')).toHaveLength(118);
  await page.goto(base);
...
test('skills are local packages with working instructions',async({page})=>{
  ...
  await expect(page.locator('[data-entry]')).toHaveCount(112);
...
test('bundle navigation links to canonical skills and a read-only command',async({page,request},testInfo)=>{
 ...
 await expect(page.locator('.bundle-card')).toHaveCount(18);
 expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBeTruthy();
 ...
 const bundles=(await data.json()).bundles;expect(bundles).toHaveLength(18);
 expect(new Set(bundles.flatMap((b:{skills:string[]})=>b.skills)).size).toBe(112);
});
